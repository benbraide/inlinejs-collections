...
class CollectionConcept {
    constructor(name_, component_, options) {
        ...
        this.keyedProxy_ = (0, inlinejs_1.CreateInplaceProxy)((0, inlinejs_1.BuildGetterProxyOptions)({
            getter: (prop) => {
                if (prop) {
    ...
    Import({ list, incremental, alertType }) {
        ...
        (0, inlinejs_1.WaitPromise)(list, (list) => {
    ...
    UpdateItem({ index, quantity, entry, incremental, alertType, callback }) {
        ...
        if (!item) { //Initialize item
            if (!entry) { //Invalid item
                return (0, inlinejs_1.JournalWarn)('Cannot update an invalid item.', 'InlineJS.CollectionConcept.UpdateItem');
            }
        ...
        (0, inlinejs_1.AddChanges)('set', `${this.id_}.items.${index}.quantity`, 'quantity', (_a = this.component_) === null || _a === void 0 ? void 0 : _a.GetBackend().changes);
        if (callback) {
            callback(item);
        }
    ...
    LeaveQueueMode_() {
        if (this.queuedTasks_) {
            this.queuedTasks_.forEach(task => (0, inlinejs_1.JournalTry)(task, 'InlineJS.CollectionConcept.QueuedTask'));
            this.queuedTasks_ = null;
        }
    }
    GetItems_() {
    ...
    CreateItemProxy_(entryName, index, getItem) {
        return (0, inlinejs_1.CreateInplaceProxy)((0, inlinejs_1.BuildProxyOptions)({
            getter: (prop) => {
                var _a, _b, _c;
                if (prop === 'quantity') {
    ...
    AlertUpdate_() {
        var _a;
        (0, inlinejs_1.AddChanges)('set', `${this.id_}.items.length`, 'length', (_a = this.component_) === null || _a === void 0 ? void 0 : _a.GetBackend().changes);
